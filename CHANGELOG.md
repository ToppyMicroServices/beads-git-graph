...
- Show Automatic, Preferred, and Pinned dispatch intent in the Manage execution plan, with
  per-task markers and counts. Tasks without an assigned target default to Automatic.
- Read optional dispatch policies from task fields, metadata, or labels. Explicit targets
  default to Preferred; execution still requires the existing Start AI confirmation.

### Fixed

- Label session-observed provider and model information as Observed to distinguish it from
