...
tags, merges, uncommitted changes, commit details, and file diffs.

A manual Git Graph refresh fetches all configured remotes by default. The toolbar shows the last
successful fetch time. Labels such as `origin/main` are local remote-tracking refs, not a live view
